...
public class WeekView extends View {
    ...
    // others
    private DBSettingHandler db = null;
    private int startVal = 9;
    ...
    private final GestureDetector.SimpleOnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            if (mCurrentFlingDirection == Direction.VERTICAL){
                mScroller.fling(0, (int) mCurrentOrigin.y, 0, (int) velocityY, 0, 0, (int) -(mHourHeight * (24 - startVal) + mHeaderTextHeight + mHeaderRowPadding * 2 - getHeight()), 0);
            }
    ...
    private void init() {
        db = new DBSettingHandler(mContext);

        if (db.getRowCount() > 0) {
            CSetting cs = db.getRow(1);
            if (cs.getTimeStart() != null) {
                if (cs.getTimeStart().length() > 1) {
                    startVal = Integer.valueOf(cs.getTimeStart().substring(0, 2));
                } else {
                    startVal = Integer.valueOf(cs.getTimeStart().substring(0, 1));
                }
            }
        }
        ...
        mToday.set(Calendar.MONTH, 4);
        mToday.set(Calendar.DATE, 4);
        ...
        mTodayBackgroundPaint.setColor(mDayBackgroundColor);
        ...
        mTodayHeaderTextPaint.setColor(mHeaderColumnTextColor);
    ...
    private void drawTimeColumnAndAxes(Canvas canvas) {
        ...
        if (mCurrentScrollDirection == Direction.VERTICAL) {
            ...
            else if (mCurrentOrigin.y - mDistanceY < -(mHourHeight * 24 + mHeaderTextHeight + mHeaderRowPadding * 2 - getHeight())) mCurrentOrigin.y = -(mHourHeight * (24 - startVal + 1) + mHeaderTextHeight + mHeaderRowPadding * 2 - getHeight());
        ...
        for (int i = startVal; i < 24; i++) {
            float top = mHeaderTextHeight + mHeaderRowPadding * 2 + mCurrentOrigin.y + mHourHeight * height + mHeaderMarginBottom;
            ...
            if (top < getHeight()) {
                canvas.drawText((height + 1) + "교시", mTimeTextWidth + mHeaderColumnPadding, top + mTimeTextHeight, mTimeTextPaint);
                //canvas.drawText("(" + time + ")", mTimeTextWidth + mHeaderColumnPadding, top + mTimeTextHeight + 30, mTimeTextPaint);
            }
            height++;
        }
    }
        ...
        if (mEventRects != null && mEventRects.size() > 0) {
            for (int i = 0; i < mEventRects.size(); i++) {
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date)) {
                    ...
                    // Calculate top.
                    float top = mHourHeight * 24 * mEventRects.get(i).top / 1440 - (mHourHeight * startVal) + mCurrentOrigin.y
                            + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight/2 + mEventMarginVertical;
                    ...
                    bottom = mHourHeight * 24 * bottom / 1440 - (mHourHeight * startVal) + mCurrentOrigin.y
                            + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight/2 - mEventMarginVertical;
...
//                    if (hour > 12) hour -= 12;
                    if (db.getRowCount() > 0) {
                        CSetting cs = db.getRow(1);
                        if (cs.getTimeStart() != null) {
                            amPm = cs.getTimeStart().split(":")[1];
                        }
                    }
                    return String.format("%02d:%02d", hour, ((amPm != null)? Integer.valueOf(amPm) : 0));
                }
            };
        }
...
//                ViewCompat.postInvalidateOnAnimation(WeekView.this);
    ...
    public void goToHour(double hour){
        ...
        if (hour < 0)
            verticalOffset = 0;
        else if (hour > (24 - startVal))
            verticalOffset = mHourHeight * (24 - startVal);

        if (mAreDimensionsInvalid) {
            mScrollToHour = hour;
            return;
        } else if (verticalOffset > mHourHeight * (24 - startVal) - getHeight() + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom)
            verticalOffset = (int)(mHourHeight * (24 - startVal) - getHeight() + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom);
...
//        return dayOne.get(Calendar.YEAR) == dayTwo.get(Calendar.YEAR) && dayOne.get(Calendar.DAY_OF_WEEK) == dayTwo.get(Calendar.DAY_OF_WEEK);
//        return dayOne.get(Calendar.DAY_OF_WEEK) == dayTwo.get(Calendar.DAY_OF_WEEK);
    }
}
